...
//Guardo en una constante llamada index una instancia de express
const app = express();
...
//Routes
app.get('/', (req, res) => res.send(posts.sort(p => p.id)))

app.get('/:id', (req, res) => res.send(posts.find(g => g.id == req.params.id)))

app.post('/', (req, res) => {
    let postToAdd = req.body;
    postToAdd.id = posts.sort(p => p.id)[posts.length -1].id + 1;
    posts.push(postToAdd)
    res.send(JSON.parse(`{"id" : "${postToAdd.id}"}`))
});
